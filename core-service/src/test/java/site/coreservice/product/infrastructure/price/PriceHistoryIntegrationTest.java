 ...
 * <p>
 * 홀드 서비스도 목으로 대신한다. order.completed는 pointwallet-service 컨슈머 그룹도 독립적으로
 * 구독 중이라, 실제 HoldService가 뜬 채로 두면 대응하는 홀드가 없어 그쪽에서 예외/에러 로그가 난다.
 * 이 테스트가 보려는 건 시세 적재 배선이지 예치금 흐름이 아니므로 끊어둔다.
 */
@Tag("integration")
