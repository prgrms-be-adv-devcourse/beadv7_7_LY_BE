...
/**
 * 카탈로그 조회 유스케이스. getProduct는 경매(07)·주문(06)이 소비하는 내부 계약으로,
...
public class ProductService {
    ...
    /** 내부 getProduct(2-1). 비활성도 반환한다 — 소비자(경매)가 active로 등록 가능 여부를 직접 판단한다. */
    public ProductSnapshotResult getProduct(Long productId) {
        Product product = productRepository.findById(productId)
                .orElseThrow(ProductNotFoundException::new);
