...
/**
 * LP 릴리스 시드 로더. local 프로파일 + {@code product.seed.enabled=true}일 때만 동작한다(기본 OFF). 커밋본은 플래그 미설정이라 팀원이 pull 받아 local로 켜도 이
 * 시드는 돌지 않는다. 로컬 적재 시엔 application-local.yml(커밋하지 않는 변경)에 {@code product.seed.enabled: true}와 {@code ddl-auto: update}를
 * 함께 둔다.
 * <p>
 * 무엇을 넣을지는 {@link ProductSeedData} 원장에 있고, 이 클래스는 그걸 순회하며 넣는 절차만 맡는다.
 * <p>
 * 여러 번 실행해도 안전하다. 확인 단위는 "자원 각각"이다 — 아티스트·상품은 이미 있으면 새로 만들지 않고 ID만 가져오며, 별칭은 소유자가 이미 있었더라도 별칭 자체의 존재를 따로 확인해 없는 것만 넣는다.
 * (아티스트가 있다고 별칭 넣기를 통째로 건너뛰면, 나중에 시드에 별칭을 추가해도 기존 DB엔 영영 반영되지 않는다)
 * <p>
 * 카탈로그번호가 없는 상품(부틀렉 등)은 번호 대신 (제목 + 아티스트 + 발매연도 + 발매국가 + 포맷 + 프레스구분)으로 중복을 확인한다 — 번호 없는 행끼리만 비교한다.
 */
@Slf4j
@Order(1)
@Profile("local")
...
public class ProductSeedLoader implements CommandLineRunner {
    ...
    private Optional<Product> findExistingProduct(String catalogNumber, Long artistId, String title,
                                                  String releaseCountry, int releaseYear, PressType pressType,
                                                  String format) {
