...
public class AuctionSchedule {
    private static final int MIN_EXTENSION_MINUTES = 1;
    ...
    public static AuctionSchedule from(Period period, boolean extensionEnabled, Integer extensionTime) {
        ...
        if (extensionEnabled) {
            if (extensionTime == null || extensionTime < MIN_EXTENSION_MINUTES)
                throw new IllegalArgumentException("연장 시간은 %d분 이상이어야 합니다.".formatted(MIN_EXTENSION_MINUTES));
        } else {
            extensionTime = null;
        }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof AuctionSchedule that)) return false;
