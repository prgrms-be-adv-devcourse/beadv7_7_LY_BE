 ...
 * <p>
 * 필드는 성격이 셋으로 갈린다. <b>검색이 질의로 훑는 것</b>(title·artistName·별칭 두 개),
 * <b>화면에 표시할 것</b>(coverImageUrl·발매정보·pressType 등 — 이 서비스는 데이터베이스에 연결돼 있지 않아
 * 문서 안에 값이 없으면 결과를 그릴 수 없다), <b>추천이 쓰는 것</b>(그룹 키 셋과 벡터 셋).
 * <p>
 * <b>필드를 나중에 추가하는 것</b>은 매핑 갱신만으로 되고 기존 문서에 값을 채워 넣어도 벡터는 그대로라
 * OpenAI 재호출이 없다. 반면 <b>기존 필드의 분석기·차원·유사도를 바꾸는 것</b>은 인덱스를 새로 만들고 전체
 * 재색인해야 한다 — 그래서 분석기 배정은 뒤집기 비용이 큰 결정이다.
 * <p>
 * <b>매핑에 없는 필드가 든 문서는 거부된다.</b> 값을 추측해서 넣는 대신 색인이 실패하므로, 필드를 더할 때는
 * 문서를 저장하기 전에 매핑부터 갱신해야 한다. 조용히 잘못된 타입으로 굳는 것보다 시끄럽게 실패하는 쪽을 택했다.
 * <p>
